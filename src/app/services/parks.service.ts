...
export class ParksService {
  ...
  getParks(): Observable<APIResponse> {
    return this.http.get<APIResponse>(environment.APILink);
  }
  async calculateDistance(park:ParkResponse): Promise<number> {
    const coordinates = await Geolocation.getCurrentPosition();
    return this.getDistanceInKm(coordinates.coords.latitude,coordinates.coords.longitude,parseFloat(park.lat),parseFloat(park.lng));
  }
